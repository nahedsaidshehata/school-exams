    ...
    <div class="col-12 col-lg-10 col-xl-8">
          ...
          <form action="{{ route('admin.lessons.update', $lesson) }}" method="POST" class="needs-validation" novalidate>
                ...
                >
                  ...
                  @foreach($sections as $section)
                    <option value="{{ $section->id }}" {{ old('section_id', $lesson->section_id) == $section->id ? 'selected' : '' }}>
                      {{ __('Subject') }}: {{ $section->material->name_en }} — {{ __('Section') }}: {{ $section->title_en }}
                    </option>
                  @endforeach
                </select>
            ...
            <div class="card admin-card content-card mb-3 mt-4">
              <div class="card-header fw-bold d-flex flex-wrap align-items-center justify-content-between gap-2">
                ...
                <span class="text-muted small">{{ __('Arabic only, English only, or both — as needed.') }}</span>
              </div>
              <div class="card-body">
                  ...
                  <div class="col-12 col-md-6">
                    ...
                    @if(!empty($lesson->content_updated_at))
                      <div class="text-muted small mt-1">
                        {{ __('Last updated') }}: <span class="mono">{{ $lesson->content_updated_at }}</span>
                      </div>
                    @endif
                  </div>
                </div>
      ...
      <div class="card admin-card attach-card mt-3">
        <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
          ...
          <span class="text-muted small">{{ __('OCR is enabled. The system will try to extract text automatically.') }}</span>
        </div>
        <div class="card-body">
                  ...
                  @foreach($attachments as $att)
                    <tr>
                      <td>
                        ...
                        <div class="text-muted small">{{ number_format(($att->size_bytes ?? 0) / 1024, 1) }} KB</div>
                      </td>
