                    ...
                    <form action="{{ route('admin.lessons.store') }}" method="POST" class="needs-validation" novalidate>
                            ...
                            <div class="col-12">
                                ...
                                >
                                    ...
                                    @foreach($materials as $m)
                                        <option
                                            value="{{ $m->id }}" {{ (string)$oldMaterialId === (string)$m->id ? 'selected' : '' }}>
                                            {{ $m->name_en }} @if(!empty($m->name_ar))
                                                — {{ $m->name_ar }}
                                            @endif
                                        </option>
                                    @endforeach
                                ...
                                <div
                                    class="form-hint mt-1">{{ __('Choose a subject first, then select one of its sections.') }}</div>
                            </div>
                                ...
                                >
                                    ...
                                    @if(isset($sections) && count($sections) > 0)
                                        @foreach($sections as $section)
                                            <option
                                                value="{{ $section->id }}" {{ (string)$oldSectionId === (string)$section->id ? 'selected' : '' }}>
                                                {{ $section->title_en }} @if(!empty($section->title_ar))
                                                    — {{ $section->title_ar }}
                                                @endif
                                            </option>
                                        @endforeach
                        ...
                        <div class="card admin-card content-card mb-3 mt-4">
                            <div
                                ...
                                <span
                                    class="text-muted small">{{ __('You can leave it empty and add it later from Edit.') }}</span>
                            </div>
                            <div class="card-body">
