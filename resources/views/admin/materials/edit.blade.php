...
@section('page_title', __('Edit Subject'))
@section('page_subtitle')
  {{ __('Update subject details. This affects the central content bank.') }}
@endsection
    ...
    <div class="col-12 col-lg-8 col-xl-6">
      <div class="card admin-card admin-form-card">
        <div class="card-header d-flex align-items-center justify-content-between gap-2">
          <span>{{ __('Subject Information') }}</span>
          <span class="badge text-bg-secondary">#{{ $material->id }}</span>
        </div>

        <div class="card-body">
          <form action="{{ route('admin.materials.update', $material) }}" method="POST" class="needs-validation" novalidate>
            @csrf
            @method('PUT')
      ...
      <div class="text-muted small mt-3">
        {{ __('Tip: Changing subject names may affect how content is organized for sections and lessons.') }}
      </div>
    </div>
  </div>
@endsection
