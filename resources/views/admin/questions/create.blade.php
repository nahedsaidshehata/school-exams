  ...
  <div class="row g-3">
    <div class="col-12 col-xl-10">
      <div class="card admin-card admin-form-card">
        ...
        <div class="card-body">
          <form action="{{ route('admin.questions.store') }}" method="POST" id="questionForm" class="needs-validation" novalidate>
            @csrf
            ...
            <div class="row g-3">
              ...
              <div class="col-12 col-md-6">
                ...
                <select id="type" name="type" class="form-select @error('type') is-invalid @enderror" required onchange="handleTypeChange()">
                  <option value="">{{ __('Select Type') }}</option>
                  <option value="MCQ" {{ old('type') == 'MCQ' ? 'selected' : '' }}>{{ __('Multiple Choice (MCQ)') }}</option>
                  <option value="TF" {{ old('type') == 'TF' ? 'selected' : '' }}>{{ __('True/False') }}</option>
                  <option value="ESSAY" {{ old('type') == 'ESSAY' ? 'selected' : '' }}>{{ __('Essay') }}</option>
                  <option value="CLASSIFICATION" {{ old('type') == 'CLASSIFICATION' ? 'selected' : '' }}>{{ __('Classification') }}</option>
                  <option value="REORDER" {{ old('type') == 'REORDER' ? 'selected' : '' }}>{{ __('Reorder') }}</option>
                  <option value="FILL_BLANK" {{ old('type') == 'FILL_BLANK' ? 'selected' : '' }}>{{ __('Fill in the Blank') }}</option>
                </select>
              ...
              <div class="col-12 col-md-6">
                ...
                <textarea id="prompt_en" name="prompt_en" rows="4" class="form-control @error('prompt_en') is-invalid @enderror" required>{{ old('prompt_en') }}</textarea>
              ...
              <div class="col-12 col-md-6">
                ...
                <textarea id="prompt_ar" name="prompt_ar" rows="4" class="form-control @error('prompt_ar') is-invalid @enderror" required>{{ old('prompt_ar') }}</textarea>
